...
(function ($) {
    ...
    var buildStructureFromLines = function (lines) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line_struct["nodes"].length != 0)
                nodes = nodes.concat([line_struct["nodes"]]);
                ...
                switch (start_elem) {
                    ...
                    case ">":
                        if (line_nodes.length === 0)
                            throw "arrow in line before node: ".concat(start_elem).concat(element_source);
                        line_arrows.push(parseSingleArrow(start_elem, previous_node_id, element_source, false));
                        break;
                }
                line = line.substring(end_pos + 1);
            }
            if (line_arrows.length !== 0 && line_arrows[line_arrows.length - 1]["target_nodes"].length === 0)
                throw "last arrow in line has no target";
    ...
    var handleEscapes = function (source) {
        while (true) {
            ...
            if (esc_end < esc_start)
                throw("escaped text without proper end. '}}' must occure in same line. '" + source.substring(esc_start) + "'");
    ...
    var parseSingleArrow = function (arrow_type, source_node_id, source, require_targets) {
        ...
        if (require_targets) {
            ...
            if (target_node_end < target_node_start)
                throw "arrow definition not closed properly: " + original_source;
            if (target_node_end != source.length - 1)
                throw "don't know what to do with '" + source.substring(target_node_end + 1) + "' at end of an arrow definition '" + original_source + "'";
    ...
    var endposForElement = function (elem, source) {
        if (ends_with[elem] === undefined || ends_with[elem].length === 0)
            throw "no end characters for element starting with '" + elem + "'";
        var endpos = -1;
    ...
    var poweredBy = function (pre_code) {
        ...
        return $('<div class="' + cssclass("powered-by") + '"><a href="' + editor_url + '?g=' + pre_code + '">Edit<span> in preview editor</span></a><span> | <a href="http://www.headjump.de/article/arrows-and-boxes" ' +
                ...
                function () {
                    $(this).addClass(cssclass("powered-by-hovered"));
                },
                function () {
                    $(this).removeClass(cssclass("powered-by-hovered"));
                }
        );
    }
    ...
    var structureForNode = function (node) {
        ...
        res.hover(function () {
            $(this).addClass(cssclass("node-hovered"));
        },
                function () {
                    $(this).removeClass(cssclass("node-hovered"));
                });
        return wrapper;
    }
    ...
    var drawArrow = function (arrow, target, outer_wrapper, pos) {
        ...
        if (sp == undefined)
            throw "unknown source_node: " + arrow["source_node"];
        for (var i = 0; i < arrow["target_nodes"].length; i++) {
            var t = arrow["target_nodes"][i];
            var tp = pos[t];
            if (tp == undefined)
                throw "unknown target_node: " + arrow["source_node"];
            ...
            if (arrow["label"] != "") {
                ...
                lab.hover(function () {
                    $(this).addClass(cssclass("label-hovered"));
                },
                        function () {
                            $(this).removeClass(cssclass("label-hovered"));
                        });
    ...
    var bulkReplace = function (source, replaces) {
        if (replaces.length % 2 !== 0)
            throw "bulkReplace error: invalid replace pairs";
    ...
    var createArrowsAndBoxes = function (ind, source) {
        ...
        try {
            ...
            $(el.parent()).hover(
                    function () {
                        $(this).addClass(cssclass("wrapper-hovered"));
                    },
                    function () {
                        $(this).removeClass(cssclass("wrapper-hovered"));
                    }
            );
        } catch (e) {
